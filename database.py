...
class Constructor :
    ...
    def load_ldc(self, dim_1_alpha=True, dim_2_alpha=True, dim_0_dtm=True, dim_1_dtm=True) :
    ...
    def standardize(self) :
        ...
        with h5py.File(self.path, 'r') as dtb : 
            ...
            out.create_dataset('y_train', data=dtb['y_train'].value[idt]-1)
            ide = shuffle(range(dtb['y_valid'].shape[0]))
            out.create_dataset('y_valid', data=dtb['y_valid'].value[ide]-1)
    ...
    def build_database(self, fft=True, quaternions=True, landscape=False, standardize=True) :

        # Compute the FFT of each normed signal
        if fft : self.load_fft()
        # Compute the quaternions based on the gyrometer
        if quaternion : self.load_qua()
        # Compute the relative landscapes based on the tri-axial signals
        if landscapes : self.load_dc()
