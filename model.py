...
class DModel :
    ...
    # Lauch the fit
    def learn(self, output, dropout=0.33, patience=4, verbose=1, max_epochs=100) :
        ...
        early = EarlyStopping(monitor='val_acc', min_delta=1e-5, patience=patience, verbose=0, mode='max')
        check = ModelCheckpoint(output, period=2, monitor='val_acc', save_best_only=True, mode='max', save_weights_only=False)
        # Fit the model
        ...
        # Load the best saved model / Overwrite the last model
        self.model = load_model(output)
        # Returns the object
        return self
